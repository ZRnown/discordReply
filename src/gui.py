...
class MainWindow(QMainWindow):
    ...
    def refresh_runtime_contexts_from_workspaces(self):
        ...
        if self.workspaces and 0 <= self.active_workspace_index < len(self.workspaces):
    ...
    def toggle_auto_scroll(self, state):
        """切换自动滚动"""
        self.auto_scroll_log = self.is_checked_state(state)

    @staticmethod
    def is_checked_state(state) -> bool:
        """兼容 PySide6 传入 int 或 CheckState 的复选框状态。"""
    ...
    def on_rotation_enabled_changed(self, state):
        """轮换启用状态改变"""
        enabled = self.is_checked_state(state)
